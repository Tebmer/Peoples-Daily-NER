...
def val(config, model):
    ...
    for index, data in enumerate(dataloader):
        ...
        for idx, (corpus, label, length) in enumerate(zip(corpus_, label_, length_)):
            ...
            # 不考虑pad以及O的标签。
            lab = label[:len(tmp)].cpu().tolist()
            pred = predict[:len(tmp)]
            lab_no_zero = []
            pred_no_zero = []
            for l, p in zip(lab, pred):
                if l != 0 :
                    lab_no_zero.append(l)
                    pred_no_zero.append(p)

            preds.extend(lab_no_zero)
            labels.extend(pred_no_zero)
